...
const AvailableMenu = ({currRestaurant_id} : {currRestaurant_id : string}) => {
  ...
  return (
    <div className="max-w-7xl mx-auto min-h-screen rounded-md px-4">
      <h1 className="text-xl md:text-2xl font-extrabold mb-6">Available Menu</h1>
      
      {/* Grid Layout - Better for responsive design */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {
          availableMenu?.length === 0 ? (
            ...
            availableMenu?.map((menu: MenuItem, idx: number) => (
              <Card
                key={idx}
                className="bg-white shadow-xl w-full max-w-[280px] mx-auto h-[350px] rounded-xl overflow-hidden flex flex-col justify-between"
              >
                <div className="-mt-6">
